---
title: "Análisis espacial Altamira"
author:
  name: Dr. Sebastián Riera
...
```{r AltaDerechos, message=FALSE}
# library(sf) ## Already loaded
AltaDerechos <- st_read(system.file("/shapes/CetDerechos/Cat_perm_derechos_Altamira_2018.shp", package="sf"), quiet = TRUE)
AltaParaje <- st_read(system.file("/shapes/UsosSuelo/ParajeAltamira/ParajeAltamira.shp", package="sf"), quiet = TRUE)
AltaUso <- st_read(system.file("/shapes/UsosSuelo/Altamira/GEE_Zona_Altamira.shp", package="sf"), quiet = TRUE)
...
```{r AltaDerechos_dplyr}
AltaDerechos %>%
  filter(categoria1 %in% c("DEFINITIVO", "PERMISO PRECARIO", "EVENTUAL", "DESAGUE")) %>%
  mutate(sup_emp1_1000 = sup_emp1*1000) %>%
  select(categoria1, contains("sup_emp1"), everything())
```
-->
...
```{r eval=F}
#tidycensus::census_api_key("65db14f4d6c0579352c4e3b07bf56ceca0ccd584", install = T)
```
...
census_api_key("65db14f4d6c0579352c4e3b07bf56ceca0ccd584", install = T)
```
