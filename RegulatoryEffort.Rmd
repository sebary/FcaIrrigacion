...
\vspace{-.5cm}
\section{Estimación de oferta hídrica}
\vspace{-.5cm}
...
\addplot[red,domain=1.5:6,semithick]{exp(x-3)/2+1.9};
\addplot[black, domain=0:1.5]{2};
...
Representación de la oferta de agua $\mathbb{A}_i^O$
\end{center}

\vspace{-.5cm}
\section{Estimación de demanda de agua para riego}
\vspace{-.5cm}
...
\begin{axis}[axis line style=gray,
	...
	axis y line=center,
	xlabel=$Hm^3 aplicados$,ylabel=$P_{Hm^3}$]
\addplot[blue,domain=0:6,semithick]{(x-8)^2/6};
\addplot[black, domain=0:6]{(x-6)^2/6};
...
\addplot[green, domain=-5:0]{-1/35*(x+6)^(3)-0.5};
\addplot[green] coordinates {(-6,-.7)} node{$PMg_i^{AE}$};
\addplot[black] coordinates {(-6,.4)} node{$CMg_{Agua}$};
\addplot[black] coordinates {(1,-4)} node{$Hm^3 \ efectivos$};
\path (axis cs:0,0) node [anchor=north west,yshift=0.7cm] {0};
\end{axis} \end{tikzpicture} \\
Representación de cambios en la demanda de agua $\mathbb{A}_i^D$ acorde a la expansión de la oferta de riego $\mathbb{A}_1^S$
\end{center}
...
\vspace{-.5cm}
\subsection{Modelo económico aplicado}
