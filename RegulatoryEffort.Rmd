...
El ahorro de agua se calcula siguiendo la siguiente fórmula,

\vspace{-.5cm}
\begin{equation}
\mathbb{A}_i^{O} = Q_{m^{3}/año} \times Ef.Cond_j \times \left[ metros \times \left( 1 - F_{m^{3}/s/km} \right) \right] 
\end{equation}

El ahorro de agua del año i ($\mathbb{A}_i^O$) equivale al caudal adicional por las obras de infraestructura, el mismo es calculado considerando el caudal promedio mensual por cauce ($j$) ponderado por la eficiencia en la conducción ($Ef.Cond_j$) y las pérdidas por infiltración.

```{r Revestimiento, results='asis'}
```

En el caso del Río Mendoza, durante el 2018 y 2019 se revistieron 
`r Revestimiento[[4,3]]` metros,
`r Revestimiento[[4,1]]` en el 2018 y
`r Revestimiento[[4,2]]` 2019, respectivamente.
