...
- Las obras de infraestructura de impermeabilización de canales se traducen a una disminución en las pérdidas por filtración en el trazado del sistema hídrico provincial.

- Se postulan dos enfoques de valorización del incremento en la eficiencia de conducción en cada cauce ($EfC_i$): eficiencia de conducción por kilómetro de la unidad de manejo (enfoque de pérdidas) y ganancia relativa de la eficiencia con respecto al estado de infraestructura (diferencias en eficiencia de conducción). El primero es más abarcativo por la información requerida pero menos detallista. El último es más exacto pero requiere información específica del aforo de los cauces. 

- Río Mendoza (2017-2020): el precio medio por USD/$m^3$ 0.857 es similar en ambos enfoques 
. El valor que se obtiene con mayor frecuencia es USD/$m^3$ `r round(median(!is.na(Mendoza$valuePerdBis)), digits=4)`.

- Rio Tunuyán Superior (`r min(Superior$Ano)`-`r max(Superior$Ano)`): el precio medio por USD/$m^3$ 0.952 y el valor con mayor frecuencia es USD/$m^3$ `r round(median(!is.na(Superior$valuePerdBis)), digits=4)`

  - Se avanza en el cálculo para toda la cuenca pero discriminando en cuatro (4) subsistemas:      
     - Las Tunas
     - Arroyo Grande
     - Dique Valle de Uco
     - Yaucha-Aguanda

- Río Tunuyán Inferior (`r min(Inferior$Ano)`-`r max(Inferior$Ano)`): el precio medio por USD/$m^3$ `r round(mean(!is.na(Inferior$valuePerdBis),na.rm=T), digits=4)`

- Ganancia en eficiencia de conducción varía entre 10-22% en las cuencas según los análisis actuales. Será importante revisar y ajustar los valores de caudales y eficiencias para concluir el trabajo desde la perspectiva económica
...
El punto de partida de análisis se sustenta en dos pilares de información: el estado de la infraestructura y las inversiones realizadas.
La primera incluye los relevamientos y estimaciones más actualizadas de condiciones de la red de riego en términos de superficie impermeabilizada y la distribución del recurso.
...
La misma permite la estimación de la disminución de la pérdida por infiltración debido a la ejecución de obras de infraestructura acompañando de información monetaria para la valorización económica.
La segunda fuente de información son las inversiones en revestimiento de cauces otorgada por el DGI, donde se discriminaron las obras de revestimiento en las subdelegaciones correspondientes y se asignaron los valores de eficiencia de conducción según la información recibida. 
La vinculación de los datos se realizó utilizando los códigos de cauce [@DGI2004].
Cuando no hay registro de aforo en el cauce revestido, se reemplaza el valor de $EfC$ por el correspondiente al estimado para la Unidad de Manejo (UM).

```{r SumFull}
```


# Subdelegación Mendoza
\vspace{-.2cm}
