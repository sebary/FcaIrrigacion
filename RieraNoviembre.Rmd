...
Dentro del sistema de conducción, la mayor proporción del total relevado corresponde a trazado sobre tierra (
`r paste(round(redtotaltip[8,1],1),'%')`
).
Seguido por datos que resta validar (
`r paste(round(redtotaltip[8,6],1),'%')`) y hormigón (`r paste(round(redtotaltip[8,2],1),'%')`)
...
\begin{wrapfigure}{l}{0.4\textwidth}
    \includegraphics[width=0.4\textwidth]{figure/tunsup.pdf}
\end{wrapfigure}
...
```{r figPrelim,results='hide'}
```

```{r subSistemas, results='asis'}
```

```{r ahorroSist2, results='hide'}
```

```{r ahorroSist, results='asis'}
```
