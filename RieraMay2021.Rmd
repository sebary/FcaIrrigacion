...
```{r AhorroMzaPerd2, results='asis',fig.show='hold', fig.height=4.5, fig.width=11, fig.cap="\\label{AhorroMzaPerd2}Curva de ahorro de agua enfoque pérdidas",out.extra="", fig.align='center'}
```
...
Si bien las curvas de ahorro de agua por menor filtración parecen similares,
existe una diferencia en el valor medio entre ambas metodologías.
El enfoque de pérdidas en la eficiencia de conducción, el precio medio por $USD/m^3$ es
`r round(mean(Superior$vAhorro1, na.rm=T), digits=2)` y la mediana del precio es `r round(median(Superior$vAhorro1, na.rm=T), digits=2)`.
...
```{r AhorroSup4, results='hide'}
```

```{r ahorroSist2, results='hide'}
```

```{r ahorroSist1, results='asis',warning=FALSE}
```
