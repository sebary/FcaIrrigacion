...
knitr::read_chunk("FcaIrrigacion.R")
...
\vspace{-.5cm}
\begin{equation}
\mathbb{A}_i^{O} = \sum_{j=1}^{n} Q_{m^{3}/año} \times \Delta metros \times \Delta pérdida
\end{equation}

\begin{equation*}
\mathbb{A}_i^{O} = \sum_{j=1}^{n} Q_{m^{3}/año} \times Superficie \times \Delta metros \times EfC_1 \times \frac{EfC_0}{Extension \:cauce\: tierra}
\end{equation*}
...
`r round(mean(!is.na(Mendoza$valueAhorro2),na.rm=T), digits=3)`.

```{r AhorroMzaPerd2, results='asis',fig.show='hold', fig.height=4.5, fig.width=11, fig.cap="\\label{AhorroMzaPerd2}Mendoza - Curva de ahorro",out.extra="", fig.align='center'}
```
...
En el caso de no disponer la información específica requerida, se considera el valor de eficiencia de conducción en canales de tierra calculado para cada unidad de manejo;
este cálculo generaliza los valores de $EfC_{j}$ posiblemente subestimando el desempeño de la unidad de manejo (UM).

En base a los resultados de la estimación, la obra de `r a2dato` representa la mayor _disminución_ _de_ _pérdidas_ por filtración por dólar invertido desde el 2017. En otras palabras, el menor costo por volumen ahorrado de agua.
La realización de esta obra de revestimiento diminuye las pérdidas anuales por filtración en 
`r round(a2dato2, digits=0)` miles de metros cúbicos (m$^3$). 

```{r MzaTableComparacion, results='asis'}
```

En el río Mendoza la curva de costo marginal alcanzó un valor promedio de `r round(mzaVol[,1], 3)` dólares por cada m$^3$ en un kilómetro; las inversiones realizadas desde 2017 permiten ahorrar `r format(round(as.numeric(mzaVol[,2]/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros anuales debido a las inversiones en revestimiento que sumaron `r format(round(as.numeric(mzaVol[,3]/1000000), 2), nsmall=1, decimal.mark=",")` millones de dólares.
...
Aunque la información de obras de infraestructura en esta cuenca está disponible desde el año 1999, las primeras obras registradas de revestimiento fueron desde el año 2000.
A diferencia de las otras cuencas, en el Tunuyán Superior se consideraron tambien tres (3 subsistemas) de riego para el análisis de las obras de revestimiento. 
El objetivo es lograr un análisis exhaustivo incluyendo las explotación con aguas subterráneas.
Por el momento, el análisis se realizó con la información de revestimiento de cauces para la distribución de agua superficial y el precio medio por $USD/m^3$ es `r round(mean(Superior$vAhorro1, na.rm=T), digits=2)` y la mediana del precio es `r round(median(Superior$vAhorro1, na.rm=T), digits=2)`.

\begin{wrapfigure}{l}{0.5\textwidth}
  \begin{center}
    \includegraphics[width=0.55\textwidth]{figure/tunsup.pdf}
  \end{center} 
\end{wrapfigure}

En términos generales el enfoque de eficiencias de conducción es superador ya que discrimina entre los distintos tipos de cauces revestidos ($j$), asignando el valor de $EfC_{j}$ según el estudio de eficiencias en la conducción para la subdelegación del río Tunuyán Superior.
Esto permitiría acercarce aún más al valor real del costo marginal de ampliar la oferta de riego.
...
```{r ahorroSup1, results='asis',fig.show='hold', fig.height=4.5, fig.width=11, fig.cap="\\label{ahorroSup1}Curva de ahorro de agua enfoque eficiencia de conducción",out.extra="", fig.align='center'}
```

En la cuenca del Tunuyán Superior la curva de costo marginal alcanzó un valor promedio de `r round(supVol[,1],3)` dólares por cada m$^3$ en un kilómetro; las inversiones realizadas desde 2000 permiten ahorrar `r format(round(as.numeric(supVol[,2]/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros anuales debido a las inversiones en revestimiento que sumaron `r format(round(as.numeric(supVol[,3]/1000000), 2), nsmall=1, decimal.mark=",")` millones de dólares.

```{r SupTableLandscape, results='asis'}
```

### Subsistemas en Tunuyán Superior

Como una alternativa para considerar la heterogeneidad del terreno y las fuentes del recurso hídrico se propone comparar esta metodología de valoración para toda la cuenta y los subsistemas propuestos. Los sistemas se organizaron como resultado de entrevistas con expertos y con soporte a la zonificación de las aguas subterráneas en el Tunuyán Superior. La figura \ref{subSistemas} presenta los subsistemas: Las Tunas, Arroyo Grande y dique Valle de Uco y Yaucha-Aguanda.

```{r subSistemas, results='asis',fig.show='hold', fig.height=4.5, fig.width=11, fig.cap="\\label{subSistemas}Visualizacion por subsistemas Tunuyan Superior",out.extra="", fig.align='center'}
```


```{r ahorroSist3, results='asis',fig.show='asis', fig.height=3.5, fig.width=5, fig.cap="\\label{ahorroSist3}Costo marginal del agua ahorrada (CMgA) por sub-sistemas en Tunuyán Superior",out.extra="", fig.align='center', warning=FALSE}
```

El tratamiento y análisis de la información permite analizar las curvas por subsistemas para comparar los ahorros logrados por revestimiento de cauces desde el año 2000 a la fecha.
El valor promedio del costo de ahorro de agua por kilómetro para los subsistemas es `r mediaTunas` para Las Tunas, `r mediaVdU` arroyo Grande y dique Valle de Uco y, por último, `r mediaYA` para el subsistema de Yaucha-Guanda.

```{r ahorroSist4, results='hold',fig.show='asis', fig.height=3.5, fig.width=5, fig.cap="\\label{ahorroSist4}Densidad CMgA por sub-sistemas",out.extra="", fig.align='center',warning=FALSE}
```

Los valores promedio por subcuenca son menores al promedio de la cuenca ( `r round(supVol[,1],3)`) lo que puede ser parcialmente explicado por el hecho que se realizaron obras donde la ganancia en eficiencia de conducción fue relativamente muy cara.
...
En la cuenca del Tunuyán Inferior la curva de costo marginal alcanzó un valor promedio de `r infVol[,1]` dólares por cada mil m$^3$ en un kilómetro; las inversiones realizadas desde 1998 permitien ahorrar `r format(round(as.numeric(infVol[,2]/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros anuales debido a las inversiones en revestimiento que sumaron `r format(round(as.numeric(infVol[,3]/1000000), 2), nsmall=1, decimal.mark=",")` millones de dólares. 

```{r ahorroInf2, results='asis',fig.show='hold', fig.height=4.5, fig.width=11, fig.cap="\\label{ahorroInf2}Curva de ahorro de agua - Tun. Inferior",out.extra="", fig.align='center'}
```
...
Se consideraron inversiones en revestimiento para las tres cuencas desde 1997, con excepción al río Mendoza cuya información está disponible desde 2017, el volumen de agua ahorrado por menores pérdidas equivale a `r  format(round(as.numeric((mzaVol[,2]+supVol[,2]+infVol[,2])/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros anuales (Hm$^3$). 
En el río Mendoza los ahorros estimados suman `r format(round(as.numeric(mzaVol[,2]/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros anuales, en la cuenca del Tunuyán Superior `r format(round(as.numeric(supVol[,2]/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros y en el Tunuyán Inferior `r format(round(as.numeric(infVol[,2]/1000000), 2), nsmall=1, decimal.mark=",")` hectómetros. 
Estas estimaciones evidencian la importancia de ejecutar obras de infraestructura que contribuyan al manejo del sistema de riego provincial.
